...
/**
 *
 * @author Nguyen Tien Linh
 * Class ServerThread is a thread that connect all client thread together
 */
public class ServerThread implements Runnable {
    ...
    public void run() {
        try {
            ...
            Server.serverThreadBus.mutilCastSend("global-message"+","+"Server: Client "+this.clientNumber+" online");
            String message;
            while (!isClosed) {
                message = is.readLine();
                if (message == null) {
            ...
            Server.serverThreadBus.sendOnlineList();
            Server.serverThreadBus.mutilCastSend("global-message"+","+"Server: Client "+this.clientNumber+" đã thoát");
        }
    }
    
    //Hàm ghi 1 dòng ra bộ đệm
